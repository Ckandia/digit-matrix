    ...
    source: {
        ...
        define: {
            'process.env': {
                ...
                // FIX: pass the legacy WebSocket app id into the build. Without this line the
                // env var set in Vercel never reaches the site and appId.js falls back to
                // 1089, which Deriv rejects on real domains (localhost-only test id).
                NEXT_PUBLIC_DERIV_LEGACY_APP_ID: JSON.stringify(process.env.NEXT_PUBLIC_DERIV_LEGACY_APP_ID ?? ''),
                ...
                // Shared secret so the frontend can call protected backend endpoints
                BACKEND_API_KEY: JSON.stringify(process.env.BACKEND_API_KEY ?? ''),
            },
        },
        alias: {
            ...
            // so @/services/backend-api resolves correctly
